...
export class FirstPageGblComponent {
  ...
  getTotalPriceIBF() {
    // Preparasi
    this.maintenanceService.getTotalPrice(19).subscribe(
      (res: any) => {
        this.totalPricePreparasi = res.data[0].total_price
      }
    )
    // Packing_in
    this.maintenanceService.getTotalPrice(20).subscribe(
      (res: any) => {
        this.totalPricePacking_in = res.data[0].total_price
      }
    )
    // Packing_out
    this.maintenanceService.getTotalPrice(21).subscribe(
      (res: any) => {
        this.totalPricePacking_out = res.data[0].total_price
      }
    )
    // Filling
    this.maintenanceService.getTotalPrice(22).subscribe(
      (res: any) => {
        this.totalPriceFilling = res.data[0].total_price
      }
    )
    // Electrical
    this.maintenanceService.getTotalPrice(23).subscribe(
      (res: any) => {
        this.totalPriceElectrical = res.data[0].total_price
      }
    )
    // Refurbished
    this.maintenanceService.getTotalPrice(24).subscribe(
      (res: any) => {
